...
def DoIt():
    ...
    for key in constraintMapping:
        for i in range(len(constraintMapping[key])):
            constraintMapping[key][i] = constraintMapping[key][i].replace(bindBoneKey, bindBonePrefix)
    ...
    try:
        ...
        for i in range(len(sortedConstraintMapping)):
            ...
            for fj in sortedConstraintMapping[i][1]:
                print("fj", fj)
                # fromJoint = fj.replace("[u'","").replace("']","")
                fromJoint = fj.replace(bindBonePrefix, "mixamorig:")
    ...
    for i in range(len(sortedConstraintMapping)):
        ...
        for rigJoint in sortedConstraintMapping[i][1]:
    ...
    simpleHandlesToFix = ["index", "middle", "thumb", "ring", "pinky", "neck", "leftfoot_fk", "rightfoot_fk",
                          "lefttoebase_fk", "righttoebase_fk", "leg_fk"]
    controlsToHide = ["spine2"]
    circleControlsToScaleUp = ["spine1", "root", "spine_"]

    for control in controls:
        namelowercase = control.lower()

        # fingers or similar controls
        for handleName in simpleHandlesToFix:
            if handleName in namelowercase:
                SelectAndScaleWithCluster([control + ".cv[0:2]", control + ".cv[4:7]"], 100)
                break

        for handleName in controlsToHide:
            if handleName in namelowercase:
                maya.cmds.setAttr(control + ".dispGeometry", False)
                break

        for handleName in circleControlsToScaleUp:
            if handleName in namelowercase:
                SelectAndScaleWithCluster(control + ".cv[0:7]", 100)
                break

        if "shoulder" in namelowercase:
            print(namelowercase + " is shoulder")
...
def SelectAndScaleWithCluster(objects, scale):
    try:
        maya.cmds.select(objects, r=True)
        return CreateClusterFromSelectionAndScale(scale)
    except:
        print("could not find objects: " + objects)
        pass


def CreateClusterFromSelectionAndScale(scale=100):
    ...
    maya.cmds.scale(scale, scale, scale, r=True, pivot=pos)
